...
def generate_env(openai_key: str, openai_base_url: str, band_human_key: str = "", model: str = "") -> str:
    """Generate .env file."""
    band_line = f"BAND_HUMAN_API_KEY={band_human_key}\n" if band_human_key else ""
    model_line = f"SUPABAND_MODEL={model}\n" if model else ""
    return (
...
def interactive_prompt() -> tuple[str, str, str]:
    ...
    model = prompt_required("Model", "deepseek-chat")
    print(f"  (Common: deepseek-chat, gpt-4o, claude-sonnet-4, gemini-2.0-flash)")
    print("")
    ...
    print(f"  LLM Model:    {C_CYAN}{model}{C_RESET}")
    ...
    return openai_key, openai_base, band_key, model
...
""",
    ...
    parser.add_argument("--model", default="deepseek-chat", help="LLM model to use (e.g. deepseek-chat, gpt-4o)")
    ...
    if args.non_interactive:
        ...
        openai_key, openai_base, band_key, model = interactive_prompt()
    ...
    env_content = generate_env(openai_key, openai_base, band_key, model)
